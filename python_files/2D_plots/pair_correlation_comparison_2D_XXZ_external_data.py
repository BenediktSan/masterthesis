...
if os.path.exists("build/plots2D/pair_correlation_comparison_XXZ_external_data") == False:
    os.mkdir("build/plots2D/pair_correlation_comparison_XXZ_external_data")
...
def find_data_for_comparison_przemek(filename):
    ...
    for file in os.listdir("../theoretical_data/2D_Heisenberg/XXZ/Przemek/"):
        if ".hdf5" in file:
            przemek_data = h5py.File("../theoretical_data/2D_Heisenberg/XXZ/Przemek/"+file, "r")
...
def find_data_for_comparison_timo(filename):
    ...
    for file in os.listdir("../theoretical_data/2D_Heisenberg/XXZ/Timo/"):
        if ".hdf5" in file:

            timo_data = h5py.File("../theoretical_data/2D_Heisenberg/XXZ/Timo/"+file, "r")
...
#calculate sum of pair correlations
    ...
    #fig.suptitle(r"Pair Correlation Comparison " + str(extract_number(pair_filename[10:])) + r" i.c. and " +str(extract_number(pair_filename[20:])) +  r"$^2$ Particles for a XXZ model")
    ...
    #axs[0, 0].plot(t_theo, get_theoretical_correlation_func(t_theo, corr_type=0), linestyle = "--", label = r"Time expansion auto correlation " )
    ...
    #axs[0, 1].plot(t_theo, get_theoretical_correlation_func(t_theo, corr_type=pair_correlation_types[1]), linestyle = "--", label = r"Time expansion $G^{zz}_{c = 1}$ " )
    axs[0, 1].legend()
    axs[0,1].set_title(r'Normalized pair correlation for $\vec{v} = (0,1)$')

        
    c = pair_correlation_types[2]
    axs[1, 0].plot(t_pair,pair_correlations[2], color = hc.pair_color_2D, label = r"Semi-classical $ G_{" + f"{c}" + r"}^{xx}$")
    ...
    #axs[1, 0].plot(t_theo, get_theoretical_correlation_func(t_theo, corr_type=pair_correlation_types[2]), linestyle = "--", label = r"Time expansion $G^{zz}_{c = 2}$ " )
    axs[1,0].legend()
    ...
    #axs[1, 1].plot(t_theo, get_theoretical_correlation_func(t_theo, corr_type=3), linestyle = "--", label = r"Time expansion $G^{zz}_{c = 3}$ " )
    axs[1, 1].legend()
    axs[1,1].set_title(r'Nomalized pair correlation for $\vec{v} = (1,1)$')
    ...
    axs[0, 0].set_ylabel(hc.pair_corr_label_2D,fontsize=size_label)
    axs[1, 0].set_ylabel(hc.pair_corr_label_2D,fontsize=size_label)

    fig.tight_layout()

    fig.savefig("build/plots2D/pair_correlation_comparison_XXZ_external_data/pair_correlation_2D_XXZ_comparison_external_data_" + system_size_name + "_" + str(extract_number(pair_filename[10:])) +"sys_" + B_field_name + ".pdf")
    pdfname = "pair_correlation_2D_XXZ_comparison_external_data_" + system_size_name + "_" + str(extract_number(pair_filename[10:])) +"sys_" + B_field_name 
    plt.savefig("build/plots2D/pair_correlation_comparison_XXZ_external_data/" + pdfname + ".pdf")
    ...
    print("\n\t\tCreating FID plot\n")
    #create FID_plot
    #aölso consider multiplicity of pair correlations

    FID_przemek = przemek_auto
    for i in range(0, len(przemek_pair)):
        multiplicity = 4


        FID_przemek = FID_przemek + multiplicity * przemek_pair[i]


    FID_timo = timo_auto
    for i in range(0, len(timo_pair) - 1):
        multiplicity = 4
        if i == 3:
            multiplicity = 8

        FID_timo = FID_timo + multiplicity * timo_pair[i]

    FID_classical_sum = auto_correlation
    for i in range(1, len(pair_correlations)):
        multiplicity = 4
        FID_classical_sum += multiplicity * pair_correlations[i]

    FID_classical_sum = sum_of_all_correlations

    FID_classical_name = pair_filename.split("pair")[0] + pair_filename.split("pair_")[1].split("s.txt")[0]


    #FID_classical, err, t_FID = np.genfromtxt("build/daten_2D/" + FID_classical_name + ".txt" , unpack=True)

    #normalize FID and rescale time
    #norm = FID_classical[1]
    #FID_classical = FID_classical / norm
    ...
    #plt.plot(t_FID , FID_classical, color = "blue",label=r"Classical FID")
    plt.plot(t_pair , FID_classical_sum, color = hc.pair_color_2D,label=r"Semi-classical FID as sum of $c \in [0,4]$")
    plt.plot(przemek_t[0] , FID_przemek, color = hc.przemek_color,label=r"Bieniek's FID")
    plt.plot(timo_t[0] , FID_timo, color = hc. timo_color_FID, label=r"Gräßer's FID")
...
for i in range(0, len(all_filenames)):
    mask[i] = "pair" in all_filenames[i] and "2D" in all_filenames[i] and "interim" not in all_filenames[i] and "variance" not in all_filenames[i] and "XXZ" in all_filenames[i]
