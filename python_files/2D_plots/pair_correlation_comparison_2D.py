...
#calculate sum of pair correlations
    ...
    if (len(pair_correlations) < correlation_cut_off * 3):
        ...
        print(f"\n\t\tAdding {(3 * correlation_cut_off) - len(pair_correlations)} empty pair correlation  to fill up subplots")
    ...
    axs[0, 0].plot(t_pair, auto_correlation, label =r"$G_0^{xx}(t)$")
    ...
    for i in range(0, correlation_cut_off):
        ...
        c = str(i + 1)
        axs[0, 1].plot(t_pair, pair_correlations[i], label = r" $m_" + f"{c}" + r" G_" + f"{c}" + r"^{xx}(t)$")
        #axs[0, 1].plot(timo_t, timo_pair[i], color = axs[0, 1].lines[-1].get_color(), linestyle = "--", label = r"$m_" + f"{c}" + r" G_" + f"{c}" + r"^{xx}$")
    axs[0, 1].legend()
    #axs[0, 1].set_title('B')
    for i in range(correlation_cut_off, 2*correlation_cut_off):
        #factor 1 due to extracted auto correlation
        c = str(i + 1)
        axs[1, 0].plot(t_pair,pair_correlations[i], label = r" $m_" + f"{c}" + r" G_{" + f"{c}" + r"}^{xx}(t)$")
        #axs[1, 0].plot(timo_t,timo_pair[i], color = axs[1, 0].lines[-1].get_color(), linestyle = "--", label = r"$m_{" + f"{c}"  + r"} G_{" + f"{c}" + r"}^{xx}$")
    axs[1,0].legend()
    for i in range(2*correlation_cut_off, 3*correlation_cut_off):
        #factor 1 due to extracted auto correlation
        c = str(i + 1)
        axs[1, 1].plot(t_pair, pair_correlations[i], label = r" $m_" + f"{c}" + r" G_{" + f"{c}" + r"}^{xx}(t)$")
        #axs[1, 1].plot(timo_t,timo_pair[i], color = axs[1, 1].lines[-1].get_color(), linestyle = "--", label = r"$m_{" + f"{c}" + r"} G_{" + f"{c}" + r"}^{xx}$")
    axs[1, 1].legend()
    #axs[1, 1].set_title('C')

    #for ax in axs.flat:
    #    ax.set(xlabel=r'$t$ / $J^{-1}$', ylabel=r'$G_{c}^{xx}$ ')
    ...
    pdfname = "pair_correlation_comparison_"+ dimension_name + system_size_name + "_" + str(extract_number(pair_filename[10:])) +"sys_" + B_field_name
