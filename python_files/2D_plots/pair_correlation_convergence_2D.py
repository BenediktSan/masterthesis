...
if os.path.exists("build/plots2D/pair_corr_convergence") == False:
    os.mkdir("build/plots2D/pair_corr_convergence")
...
def calc_difference(final_filename, interim_filename, correlation_type):
    ...
    data_final = read_txt_file("build/daten_2D/"+final_filename)  
    ...
    data_interim = read_txt_file("build/daten_2D/"+interim_filename)  
...
def plot_correlation_convergence_inlet(pdfname, system_amount, differences, N_fit, fit_params, interim_filenames):
    ...
    ax.set_ylabel(hc.deviation_2D_label_interim, fontsize = size_label)
    ...
    outpath = "build/plots2D/pair_corr_convergence/" + pdfname + ".pdf"
...
def plot_interim_difference_for_pair_corr(filename, interim_results_filename):#,FID_exp, t_exp):
    ...
    data = read_txt_file("build/daten_2D/"+interim_results_filename)  
    ...
    data_final = read_txt_file("build/daten_2D/"+filename)  
    ...
    plt.plot(t_pair , final_pair_correlations[correlation_type], color = "green",label=r"Correlation")
    ...
    if(len(system_amount) < 8 ):
        plt.legend()
    plt.tight_layout()
    pdfname = "pair_corr_comparison_"+ xxz_name + interim_results_filename[:-4]
    plt.savefig("build/plots2D/pair_corr_convergence/" + pdfname + ".pdf")
...
def plot_pair_correlation_convergence(final_filename, interim_filenames):
    ...
    if(len(interim_filenames) == 0):
        print(f"\n\nNo interim filenames found for {final_filename}\n\n")
        return

    xxz_name = ""
    if "XXZ" in final_filename:
        xxz_name = "XXZ_"
    ...
    for i in range(0, len(pair_correlation_types)): 
        ...
        print(f"\ta = {params[0]:.2f} ± {err[0]:.2f}")
    ...
    for i in range(0, len(differences)):
        ...
        if(len(B_field_names) < 10):
            file_B_field  = ""
        plt.plot(system_amount, differences[i ],"^", label=str(extract_number(interim_filenames[i][20:])) + r"$^2$ lattice sites " + file_B_field + f" c = {pair_correlation_types[i]}")
    ...
    plt.ylabel( hc.deviation_2D_label_interim)
    plt.legend() 
    plt.tight_layout()   
    pdfname = "interim_pair_deviation_2D_" + xxz_name + correlation_types_name + B_field_names + str(extract_number(interim_filenames[0][3:])) +"sys"
    plt.savefig("build/plots2D/pair_corr_convergence/" + pdfname + ".pdf")
    #check if plot is in master thesis folder and replace it if it is
    matching_paths = find_pdfs( pdfname)
    for path in matching_paths:
...
for file in all_filenames:
    ...
    for i in range(0, allready_checked_sys_amount.size):
        if system_amount == allready_checked_sys_amount[i] and B_field_name == allready_checked_B_field[i] and xxz_flag == allready_checked_XXZ[i]:
    ...
    mask = [False] * len(all_filenames)
    for i in range(0, len(all_filenames)):
        if(system_amount == extract_number(all_filenames[i][10:]) and B_field_name in all_filenames[i] and "2D" in all_filenames[i] and ("XXZ" in all_filenames[i]) == xxz_flag):
            mask[i] = True
    ...
    allready_checked_XXZ = np.append(allready_checked_XXZ, xxz_flag)
    ...
    for i in range(0, interim_name.size):
        if interim_name.size <= 1 or final_name.size == 0:
            #plot_interim_difference_for_pair_corr(final_name,interim_name[0])
            continue
        plot_interim_difference_for_pair_corr(final_name[0], interim_name[i])#, exp_111, t_111)                    
        #plot_interim_difference(file, interim_results_file)
