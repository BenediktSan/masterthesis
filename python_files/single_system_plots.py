...
for file in os.listdir("build/daten"):
    if file.endswith(".txt"):
        #print(file)
        if(file.endswith("Magnetization.txt") ==True ):
            file_amount +=1        
            print(f"\n\tFile {file} opened\n")
            plot_magnetization(file)
            continue
        if(file.endswith("Magnetization_CS.txt") ==True ):
            file_amount +=1        
            print(f"\n\tFile {file} opened\n")
            plot_magnetization_CS(file)
            continue
        if(file.endswith("Correlation.txt") ==True and not file.endswith("averaged_Correlation.txt")):
            file_amount +=1        
            print(f"\n\tFile {file} opened\n")
...
print("Dateianzahl opened for single system plots: ", file_amount)
